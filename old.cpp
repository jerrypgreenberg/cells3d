...
void CellSimulation::placeNewCell(Cell oldCell, Cell newCell, int icount) {
        double scale;
        double theta,phi;
        double ranThetaAngle,ranPsiAngle
		...
		if ((oldCell.getSubType() == SubTypes::MAIN_R) ||
			...
			(cellGrowthCount <= 2)) {
			   newCell.setCoords(Transform::translate(0., getStepLength(), 0.,ZERO_COORDS));
                 ...
                 phi   = getCalculatedZAngle(dx, dy, dz);
                 theta = getCalculatedYAngle(dx, dy, dz);
...
double CellSimulation::getCalculatedZAngle(double dx, double dy,double dz)
{
